...
export async function POST(req: Request) {
  if (!isDatabaseConfigured()) {
    return NextResponse.json({ error: "Account actions are offline." }, { status: 503 })
  }
  ...
  try {
    ...
    if (env.RESEND_API_KEY) {
      ...
      const locale = (store.get("NEXT_LOCALE")?.value as Locale | undefined) ?? "en"
      const origin = env.AUTH_URL ?? new URL(req.url).origin
      const url = `${origin}/${locale}/account/delete/confirm/${token}`
      const t = await getTranslations({ locale, namespace: "email" })
      const html = await accountDeletionHtml({ url, locale })
      await sendEmail({
        to: member.email,
        subject: t("deleteAccountSubject"),
        html,
      })
    }
